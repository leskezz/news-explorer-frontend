...
function Header({ blackTheme, clickAuthHandler, isPopupOpen, onSubmit, handleLogout, loggedIn, userName }) {
    ...
    return(
        <header className={`header ${blackTheme ? 'header_black' : 'header_white'}`}>
            <div className={`header__top ${blackTheme ? 'header__top_black' : 'header__top_white'} ${isPopupOpen && 'header__top_hide'}`}>
                ...
                <Navigation clickAuthHandler={clickAuthHandler} blackTheme={blackTheme} isMenuOpened={isMenuOpened} handleLogout={handleLogout} loggedIn={loggedIn} userName={userName} />
